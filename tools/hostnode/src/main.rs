...
//!   run --authkey K [--hostname H] [--control URL] [--port 80] [--state FILE]
//!       join the tailnet and serve the snapshot HTTP server on the port with
//!       a built-in test image (the same request handling as the firmware).
...
fn main() {
    ...
    match args.get(1).map(|s| s.as_str()) {
        ...
        Some("run") => run(&args[2..]),
        _ => {
            eprintln!("usage:\n  hostnode probe-control [URL]\n  hostnode run --authkey K [--hostname H] [--control URL] [--port 80] [--state FILE]");
            std::process::exit(2);
        }
    }
}
...
fn run(args: &[String]) {
    ...
    let mut port = adapter_core::shell::DEFAULT_HTTP_PORT;
    ...
    while i < args.len() {
        ...
        match args[i].as_str() {
            ...
            "--port" => port = v.and_then(|p| p.parse().ok()).unwrap_or(80),
    ...
    log::info!("snapshot server on tailnet port {port}; fetch with: curl -o snap.jpg http://<this-node>:{port}/");
    loop {
        ...
        std::thread::spawn(move || {
            ...
            let mut buf = [0u8; 512];
            let response = loop {
                match conn.read(&mut buf, Duration::from_secs(10)) {
                    Ok(0) => break None,
                    Ok(n) => {
                        head.extend_from_slice(&buf[..n]);
                        if let Some(len) = http::head_len(&head) {
                            break Some(respond(&head[..len]));
                        }
                        if head.len() > http::MAX_HEAD {
                            break Some(http::text_response(431, "request head too large"));
                        }
                    }
                    Err(e) if e.kind() == std::io::ErrorKind::TimedOut => {
                        break Some(http::text_response(408, "request timeout"))
                    }
                    Err(_) => break None,
                }
            };
...
/// Same routing as the firmware, with a test card instead of a camera frame.
fn respond(head: &[u8]) -> Vec<u8> {
    const TEST_CARD: &[u8] = include_bytes!("../testcard.jpg");
    ...
    match http::route(&req) {
        Route::Snapshot { head_only } => {
            for (k, v) in req.query() {
                // No camera here; settings are parsed and reported only.
                match adapter_core::uvc::parse_setting(&k, &v) {
                    Ok((def, setting)) => {
                        log::info!("setting {}={:?} (ignored)", def.name, setting)
                    }
                    Err(e) => return http::text_response(400, &e),
                }
            }
            let mut r = http::response_head(200, "image/jpeg", TEST_CARD.len());
            if !head_only {
                r.extend_from_slice(TEST_CARD);
            }
            r
        }
        Route::Controls => {
            let body = "{}\n";
            let mut r = http::response_head(200, "application/json", body.len());
