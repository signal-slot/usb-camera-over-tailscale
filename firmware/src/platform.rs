...
//! Tailscale node and the camera driver.

use crate::camera::Camera;
use crate::nvs_store::{self, NvsStore};
use crate::tls::EspDialer;
use crate::wifi::WifiManager;
use adapter_core::shell::{
    ApInfo, NodeState, Platform, SavedConfig, StatusSnapshot, DEFAULT_HOSTNAME,
};
use esp_idf_svc::sys;
...
impl Platform for EspPlatform {
    ...
    fn camera_info(&self) -> Vec<String> {
        ...
        let mut lines = vec![format!("Camera:    {}", cam.describe())];
        let formats = cam.formats();
        if formats.is_empty() {
            lines.push("Formats:   none (no camera enumerated)".into());
        } else {
            lines.push("Formats:".into());
            for f in formats {
    ...
    fn capture(&self) -> Result<String, String> {
        let cam = self.camera.as_ref().ok_or("no camera in this mode")?;
        let t = Instant::now();
        let jpeg = cam.snapshot(0).map_err(|e| e.to_string())?;
        Ok(format!(
            "captured {} bytes in {} ms ({})",
            jpeg.len(),
            t.elapsed().as_millis(),
            cam.describe()
        ))
