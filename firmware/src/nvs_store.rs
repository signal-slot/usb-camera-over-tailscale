...
use adapter_core::shell::{parse_resolution, SavedConfig, WifiNetwork};
use anyhow::Result;
...
const K_PORT: &str = "http_port";
/// `<width>x<height>`.
const K_RES: &str = "resolution";
const K_FPS: &str = "fps";
...
pub fn load_config(store: &dyn Store) -> Option<SavedConfig> {
    ...
    Some(SavedConfig {
        ...
        http_port: store.get(K_PORT).and_then(|p| p.parse().ok()),
        resolution: store.get(K_RES).and_then(|r| parse_resolution(&r)),
        fps: store.get(K_FPS).and_then(|f| f.parse().ok()),
    })
}
...
pub fn save_config(store: &dyn Store, cfg: &SavedConfig) -> std::io::Result<()> {
    ...
    set_or_remove(
        store,
        K_PORT,
        cfg.http_port.map(|p| p.to_string()).as_deref(),
    )?;
    set_or_remove(
        store,
        K_RES,
        cfg.resolution.map(|(w, h)| format!("{w}x{h}")).as_deref(),
    )?;
    set_or_remove(store, K_FPS, cfg.fps.map(|f| f.to_string()).as_deref())?;
    if identity_changed {
...
/// and has no business staying in flash.
