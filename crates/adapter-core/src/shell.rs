...
pub struct SavedConfig {
    ...
    /// TCP port of the HTTP server on the tailnet (default 80).
    pub http_port: Option<u16>,
    /// Requested frame size as (width, height). `None` picks the camera's
    /// MJPEG format closest to 640x480.
    pub resolution: Option<(u16, u16)>,
    /// Requested frame rate. `None` takes the camera's default for the size.
    pub fps: Option<u16>,
}
...
pub trait Platform {
    ...
    fn reboot(&self) -> !;
    /// Lines describing the camera: the attached device and the formats it
    /// offers. Empty when no camera driver is running (setup mode).
    fn camera_info(&self) -> Vec<String> {
        Vec::new()
    }
    /// Captures one frame and describes it (size, timing) for diagnostics.
    fn capture(&self) -> Result<String, String> {
        Err("no camera in this mode".into())
    }
}

const PROMPT: &str = "> ";
const LOGIN_WAIT: Duration = Duration::from_secs(600);
/// Tailnet hostname used until the user picks one.
pub const DEFAULT_HOSTNAME: &str = "camera";
...
impl<'a, C: Console, P: Platform> Shell<'a, C, P> {
    ...
    pub fn dispatch(&mut self, cmd: &str) {
        ...
        match verb {
            ...
            "port" => match args.first().and_then(|p| p.parse::<u16>().ok()) {
                Some(p) if p != 0 => self.set_field(|c| c.http_port = Some(p)),
                _ => self.line("usage: port <1-65535>"),
            },
            "resolution" => match args.first().copied() {
                Some("auto") => self.set_field(|c| c.resolution = None),
                Some(r) => match parse_resolution(r) {
                    Some(wh) => self.set_field(|c| c.resolution = Some(wh)),
                    None => self.line("usage: resolution <width>x<height> | resolution auto"),
                },
                None => self.line("usage: resolution <width>x<height> | resolution auto"),
            },
            "fps" => match args.first().copied() {
                Some("auto") => self.set_field(|c| c.fps = None),
                Some(f) => match f.parse::<u16>() {
                    Ok(f) if f != 0 => self.set_field(|c| c.fps = Some(f)),
                    _ => self.line("usage: fps <1-65535> | fps auto"),
                },
                None => self.line("usage: fps <1-65535> | fps auto"),
            },
            "camera" => {
                let lines = self.platform.camera_info();
                if lines.is_empty() {
                    self.line("no camera driver running (setup mode); reboot into normal mode and use the UART shell");
                }
                for l in lines {
                    self.line(&l);
                }
            }
            "snap" => match self.platform.capture() {
                Ok(d) => self.line(&d),
                Err(e) => self.line(&format!("capture failed: {e}")),
            },
            "login" => {
                self.login();
            }
    ...
    fn help(&mut self) {
        for l in [
            ...
            "status             show Wi-Fi / Tailscale / camera state",
            ...
            "resolution <WxH>   frame size to request from the camera (auto: closest to 640x480)",
            "fps <n>            frame rate to request (auto: camera default)",
            "camera             show the attached camera and its formats (normal mode)",
            "snap               capture one frame and report its size (normal mode)",
    ...
    fn status(&mut self) {
        ...
        self.line(&format!("Camera:    {}", st.camera));
        self.line(&format!("Memory:    {}", st.heap));
        if let Some(c) = self.platform.config() {
            self.line(&format!(
                "Config:    networks={} hostname={} port={} resolution={} fps={} authkey={}",
                c.networks
                    .iter()
                    .map(|n| n.ssid.as_str())
                    .collect::<Vec<_>>()
                    .join(","),
                c.hostname,
                c.http_port.unwrap_or(DEFAULT_HTTP_PORT),
                c.resolution
    ...
    pub fn wizard(&mut self, full: bool) {
        ...
        if self.login() {
            ...
            if !a.eq_ignore_ascii_case("n") {
                self.line("Rebooting. Plug the camera into the board's USB port.");
                self.platform.reboot();
            }
        }
    }
...
/// Parses `<width>x<height>` (for example `640x480`).
pub fn parse_resolution(s: &str) -> Option<(u16, u16)> {
    let (w, h) = s.split_once(['x', 'X'])?;
    let (w, h) = (w.parse::<u16>().ok()?, h.parse::<u16>().ok()?);
    if w == 0 || h == 0 {
        return None;
    }
    Some((w, h))
...
mod tests {
            ...
            *p += 1;
            match *p {
                ...
                _ => NodeState::Running {
                    ...
                    dns_name: "camera.example.ts.net".into(),
                },
            }
        }
        fn factory_reset(&self) -> Result<(), String> {
    ...
    fn wizard_end_to_end() {
        ...
        assert!(
            out.contains("Tailscale is up: camera.example.ts.net 100.64.0.9"),
            "{out}"
        );
    ...
    fn commands_and_line_editing() {
        let plat = FakePlatform::default();
        let mut con = FakeConsole {
            input: b"hostname my-host\rauthkey nope\rport 8080\rresolution 320x240\rfps 5\rresolution 12\rstatus\r"
                .to_vec()
                .into(),
            output: String::new(),
            strict: true,
        ...
        assert_eq!(cfg.resolution, Some((320, 240)));
        assert_eq!(cfg.fps, Some(5));
        assert!(con.output.contains("usage: authkey"));
        assert!(con.output.contains("usage: resolution"));
        assert!(con.output.contains(
            "Config:    networks= hostname=my-host port=8080 resolution=320x240 fps=5 authkey=none"
        ));
    ...
    fn resolution_parsing() {
        ...
        assert_eq!(parse_resolution("640x-1"), None);
        assert_eq!(parse_resolution("70000x1"), None);
    }
}
